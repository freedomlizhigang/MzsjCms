...
<body>
	...
	<form action="<?php echo U('delpos');?>" method="post">
	<table class="table-form">
		...
		<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pos): $mod = ($i % 2 );++$i;?><tr>
			<td><input type="checkbox" name="posids[]" value="<?php echo ($pos["posid"]); ?>" class="subcheck" ></td>
			<td><?php echo ($pos["posid"]); ?></td>
			<td><?php echo ($pos["name"]); ?></td>
			<td><a href="<?php echo U('editpos',array('posid'=>$pos['posid']));?>">修改</a> | <a href="<?php echo U('delpos',array('posid'=>$pos['posid']));?>" class="confirm">删除</a></td>
