...
<body>
	...
	<form action="<?php echo U('delmsg');?>" method="post">
	<table class="table-form">
		...
		<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$wxmsg): $mod = ($i % 2 );++$i;?><tr>
			<td><input type="checkbox" name="msgids[]" class="subcheck" value="<?php echo ($wxmsg["msgid"]); ?>" ></td>
			<td><?php echo ($wxmsg["msgid"]); ?></td>
			<td><?php echo ($typename[$wxmsg[msgtype]]); ?></td>
			<td><?php echo ($wxmsg["msgcon"]); ?></td>
			<td><a href="<?php echo U('editmsg',array('msgid'=>$wxmsg[msgid]));?>">修改</a> | <a href="<?php echo U('delmsg',array('msgid'=>$wxmsg[msgid]));?>" class="confirm">删除</a></td>
