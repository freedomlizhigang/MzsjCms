	...
	<form action="<?php echo U();?>" class="form-box" method="post">
		...
		<div class="form-group">
			...
			<select name="roleid" id="">
				...
				<?php if(is_array($rolelist)): $i = 0; $__LIST__ = $rolelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rlist): $mod = ($i % 2 );++$i;?><option value="<?php echo ($rlist["roleid"]); ?>"<?php if($rlist['roleid'] == $info['roleid']): ?>selected<?php endif; ?>><?php echo ($rlist["rolename"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
		</div>
		<div class="form-group">
			<label for="realname" class="item-label">真实姓名：<span class="color_9"> 必须填写</span></label>
			<input type="text" name="realname" value="<?php echo ($info["realname"]); ?>" class="input-text">
		</div>
		<div class="form-group">
			<label for="email" class="item-label">邮箱：<span class="color_9"> 必须填写</span></label>
			<input type="text" name="email" value="<?php echo ($info["email"]); ?>" class="input-text">
		</div>
		<div class="form-group">
			<label for="tel" class="item-label">电话：<span class="color_9"> 必须填写</span></label>
			<input type="text" name="tel" value="<?php echo ($info["tel"]); ?>" class="input-text">
		</div>
		<div class="form-group">
			<label for="status" class="item-label">状态：</label>
			<input type="radio" name="status"<?php if($info['status'] == 1): ?>checked="checked"<?php endif; ?> class="input-radio" value="1"> 启用
			<input type="radio" name="status"<?php if($info['status'] == 0): ?>checked="checked"<?php endif; ?> class="input-radio" value="0"> 禁用
		</div>
		<div class="form-group">
