...
<body>
	...
	<form action="<?php echo U('delmenu');?>" method="post">
	<table class="table-form">
		...
		<?php if(is_array($tree)): $i = 0; $__LIST__ = $tree;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><span class="f_l"><?php echo ($menu["nbsp"]); echo ($menu["name"]); ?></span><?php if($menu['level'] < 4): ?><a href="<?php echo U('addmenu',array('parentid'=>$menu['menuid']));?>" class="addsub"></a><?php endif; ?></td>
			<td><?php echo ($menu["url"]); ?></td>
			<td><?php echo ($menu["displayname"]); ?></td>
			<td><a href="<?php echo U('menu/editmenu',array('mid'=>$menu['menuid']));?>">修改</a> | <a href="<?php echo U('menu/delmenu',array('mid'=>$menu['menuid']));?>" class="confirm">删除</a></td>
