...
<body>
	...
	<form action="<?php echo U('dellink');?>" method="post">
	<table class="table-form">
		...
		<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$link): $mod = ($i % 2 );++$i;?><tr>
			<td><input type="checkbox" name="linkids[]" value="<?php echo ($link["linkid"]); ?>" class="subcheck" ></td>
			<td><?php echo ($link["linkid"]); ?></td>
			<td><?php echo ($link["listorder"]); ?></td>
			<td><?php echo ($link["name"]); ?></td>
			<td><?php echo ($link["url"]); ?></td>
			<td><a href="<?php echo U('editlink',array('linkid'=>$link['linkid']));?>">修改</a> | <a href="<?php echo U('dellink',array('linkid'=>$link['linkid']));?>" class="confirm">删除</a></td>
