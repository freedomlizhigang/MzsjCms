...
<body>
	...
	<table class="table-form">
		...
		<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$admin): $mod = ($i % 2 );++$i;?><tr>
			<td><input type="checkbox" name="adminids[]" class="subcheck"<?php if($admin['adminid'] == 1): ?>value="" disabled="true"<?php else: ?> value="<?php echo ($admin["adminid"]); ?>"<?php endif; ?>></td>
			<td><?php echo ($admin["adminid"]); ?></td>
			<td><a href="<?php echo U('editadmin',array('adminid'=>$admin['adminid']));?>"><?php echo ($admin["adminname"]); ?></a></td>
			<td><?php echo ($admin["realname"]); ?></td>
			<td><?php echo ($rolename[$admin['roleid']]); ?></td>
			<td><?php echo ($admin["tel"]); ?></td>
			<td><?php echo ($admin["lastip"]); ?></td>
			<td><?php echo (date('Y-m-d H:i:s',$admin["lasttime"])); ?></td>
			<td><?php echo ($admin["statusname"]); ?></td>
			<td><a href="<?php echo U('editpassword',array('adminid'=>$admin['adminid']));?>">修改密码</a> | <?php if($admin['adminid'] != 1): ?><a href="<?php echo U('deladmin',array('adminid'=>$admin['adminid']));?>" class="confirm">删除</a><?php else: ?>删除<?php endif; ?></td>
